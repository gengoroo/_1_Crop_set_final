function  [ListDim1,ListDim2,ListFrames,pn_save,note] = f_mov_crop(Listfn,pn,ListDim1,ListDim2,ListFrames,varargin)
    ...
    for ii = 1:nargin-5
        ...
        if strcmp(varargin{ii}, 'Circle3P')
            Circle3P = varargin{ii+1};
        end
    ...
    if yn_manual_crop == 'y'
        ...
        sel_crop_way = input('1:Rectangle 2:Circle\n');
        ...
        switch sel_crop_way
            case 1
                if ~isempty(ListDim1)&&~isempty(ListDim2)
                    ...
                    fprintf('Click 2 diagonal corners\n');
                    for ii = 1:2
                        [x(ii),y(ii)]= ginput (1);
                        plot(x(ii),y(ii),'r.');
                    end
            ...
            case 2
                fprintf('Click 3 points on the circle\n');
                for ii = 1:3
                    [x(ii),y(ii)]= ginput (1);
                    plot(x(ii),y(ii),'r.');
                end
                fprintf('x (CircleDIM2) is %s\n',num2str(x));
                fprintf('y (CircleDIM1) is %s\n',num2str(y));
                ListCircleDim1 = ceil(min(y)):floor(max(y));
                ListCircleDim2 = ceil(min(x)):floor(max(x));
                ABC = [x', y'];
                [r,Center] = fit_circle_through_3_points(ABC);
                viscircles(Center',r);
                Circle3P.r=round(r);
                Circle3P.Center = round(Center);

        end
    end

    if exist('Circle3P','var')
        r = Circle3P.r;
        Center = Circle3P.Center;
        MaskSize = [2*r+1;2*r+1];
        Mask = uint8(createCirclesMask(MaskSize,[r+1,r+1],r));
        ListDim1 = (Center(2) - r):(Center(2) + r);
        ListDim2 = (Center(1) - r):(Center(1) + r);
    end
    ...
    if ~isempty(Listfn) % if Listfn is specified, the files will be cropped
        ...
        for id_fn = 1:numel(Listfn)
            fn = Listfn{id_fn};
            if exist('Mask','var')
                [Listfn_out{id_fn}, FrameRate(id_fn)] = crop(fn,pn,ListDim1,ListDim2,ListFrames,'Mask',Mask,'pn_save',pn_save,'output_imsize',output_imsize);
            else
        ...
        fprintf('Cropping all files in a folder\n')
        Info = dir(pn);
        counter = 0;
    ...
    if exist('Circle3P','var')
        save([pn_save, '\List_movie_crop_data.mat'],   'List_fn_out_movie','pn_out_movie','FrameRate','ListDim1','ListDim2','Circle3P','ListFrames','note');
    else
